      ...
      return (0, _fsExtra.readFile)(filepath, readOpts)
        ...
        .then(
          data =>
            !data
              ? this.messageLeaveAs()
              : (0, _fsExtra.outputFile)(this.outpath(), data).then(() =>
                  this.messageTranslate()
                )
        )
    } else if (type === 'stream') {
        ...
        return (0, _fsExtra.ensureDir)((0, _path.resolve)(outDir)).then(
          () =>
            new Promise((resolve, reject) => {
              ...
              const writable = (0, _fsExtra.createWriteStream)(this.outpath())
              writable.on('error', reject)
              writable.on('finish', () => resolve(this.messageTranslate()))
              ...
              if (!result) {
                ...
                readable.destroy(new Error(`transform must return stream`))
              } else {
