...
const transformWeirs = (put, out, preset, files, weirs) => {
   ...
   const mixes = files.map(file => {
      ...
      if (!applicable) {
         ...
         return new _Content2.default(file, outfile, exts, preset)
      }
   })
...
const transformExts = (put, out, preset, mixes, exts) => {
